...
fun BikeShopApp() {
    ...
    Scaffold(
        ...
        NavHost(navController = nav, startDestination = "login", modifier = Modifier.padding(padding)) {
            ...
            composable("home") { HomeScreen(onBuyNow = { id -> nav.navigate("compras") }, onOpenQuienes = { nav.navigate("quienes") }, onOpenDetail = { id -> nav.navigate("detalle/$id") }, cartViewModel = cartViewModel) }
            composable("productos") { PantallaProductos(onOpen = { id -> nav.navigate("detalle/$id") }, onAgregar = { nav.navigate("agregar") }, onBuyNow = { nav.navigate("compras") }, cartViewModel = cartViewModel) }
            composable("carrito") { CartScreen(onCheckoutDone = { nav.navigate("perfil") }, cartViewModel = cartViewModel, authViewModel = authViewModel) }
